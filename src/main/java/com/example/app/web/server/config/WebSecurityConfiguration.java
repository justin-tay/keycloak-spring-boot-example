...
public class WebSecurityConfiguration {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http, JWKSet jwks,
		...
		return http
				.authorizeHttpRequests(authorizeHttpRequests -> authorizeHttpRequests
						.requestMatchers(new AntPathRequestMatcher("/oauth2/jwks")).anonymous())
				.authorizeHttpRequests(authorizeHttpRequests -> authorizeHttpRequests
						.requestMatchers(new AntPathRequestMatcher("/**")).authenticated())
				.oauth2Login(oauth2Login -> oauth2Login
						.tokenEndpoint(
								tokenEndpoint -> tokenEndpoint.accessTokenResponseClient(accessTokenResponseClient))
						...
						defaultLoginPage -> defaultLoginPage.withObjectPostProcessor(new ObjectPostProcessor<Object>() {
							...
							public <O> O postProcess(O object) {
								if (object instanceof DefaultLoginPageGeneratingFilter filter) {
									// Configure this so the default login page generates the logout message after
	...
	/**
	 * Gets the oidc logout success handler that calls the OpenID
	...
	private DefaultAuthorizationCodeTokenResponseClient accessTokenResponseClient(JWKSet jwks) {
		Function<ClientRegistration, JWK> jwkResolver = clientRegistration -> jwks.getKeys().stream()
				.filter(jwk -> KeyUse.SIGNATURE.equals(jwk.getKeyUse())).findFirst().get();
	...
	private void addAuthorities(Set<GrantedAuthority> authorities, ClaimAccessor token, String claim,
		...
		if (realmAccess != null) {
			if (realmAccess.get("roles") instanceof Collection<?> roles) {
				roles.stream().map(value -> authorityPrefix + value.toString()).map(SimpleGrantedAuthority::new)
